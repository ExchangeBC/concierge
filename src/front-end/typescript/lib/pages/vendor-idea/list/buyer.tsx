...
type FormFieldKeys = 'categoryFilter' | 'industrySectorFilter' | 'searchFilter';
...
type InnerMsg = ADT<'categoryFilter', Select.Value> | ADT<'industrySectorFilter', Select.Value> | ADT<'searchFilter', string>;
...
export const init: Init<Params, State> = async ({ sessionUser, dispatch }) => {
  ...
  if (result.tag === 'valid') {
    ...
    vis = result.value.items.sort((a, b) => {
  ...
  return (
    <div className="mb-3">
      ...
      <Row className="d-none d-md-flex align-items-end">
        <Col xs="12" md="4">
          <Select.view state={state.value.industrySectorFilter} onChange={onChangeSelect('industrySectorFilter')} />
        </Col>
        <Col xs="12" md="4">
          <Select.view state={state.value.categoryFilter} onChange={onChangeSelect('categoryFilter')} />
        </Col>
        <Col xs="12" md="4" className="ml-md-auto">
          <ShortText.view state={state.value.searchFilter} onChange={onChangeShortText('searchFilter')} />
        </Col>
      </Row>
    </div>
  );
            ...
            {state.value.visibleVis.map((vi, i) => (
              <Col xs="12" md="6" lg="4" key={`vi-list-result-buyer-${i}`} className="vi-card">
                <Card body className="p-3 h-100">
                  ...
                  <CardSubtitle className="text-secondary font-weight-bold font-size-small">{makeCategoryString(vi.latestVersion.description.categories)}</CardSubtitle>
                  <CardText className="mt-3">{truncate(vi.latestVersion.description.summary, { length: 180, omission: '...', separator: /[.!?]/ })}</CardText>
                  <CardText className="small text-secondary">Published {formatRelativeTime(vi.createdAt)}.</CardText>
                  <div className="mt-md-auto w-100 d-flex flex-nowrap">
                    <Link button outline route={{ tag: 'viView', value: { viId: vi._id } }} color="info" className="mr-2 text-nowrap" size="sm">
                      Learn More
                    </Link>
                    <Link button href={expressInterestHref(vi.latestVersion.description.title)} color="primary" size="sm" className="text-nowrap">
                      Express Interest
                    </Link>
                  </div>
                </Card>
    ...
    <div>
      <Row className="mb-5">
        <Col xs="12" md="9" lg="8">
          ...
          <p>The following list contains Unsolicited Proposals that are eligible to be purchased by government.</p>
        </Col>
      </Row>
      <Filters {...props} />
      <Results {...props} />
