...
export const init: Init<Params, State> = async ({ rfiId, userType, fixedBarBottom = 0, preview = false }) => {
  ...
  switch (rfiResult.tag) {
    case 'valid':
      ...
      const alerts: string[] = [];
      if (preview) {
        alerts.push('This is a preview. All "Published" and "Last Updated" dates on this page are only relevant to the preview, and do not reflect the dates associated with the original RFI. Previews are automatically deleted after 7 calendar days.');
      } else {
