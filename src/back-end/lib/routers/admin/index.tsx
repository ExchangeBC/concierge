...
async function makeEmailNotificationReference(): Promise<View<Record<string, never>>> {
  const notifications: NotificationGroupProps[] = [
    { title: 'Create User', emails: await mailer.createUserT(mocks.email) },
    {
      ...
      emails: [...(await mailer.deactivateUserT(mocks.email, UserType.Vendor)), ...(await mailer.deactivateUserT(mocks.email, UserType.Buyer)), ...(await mailer.deactivateUserT(mocks.email, UserType.ProgramStaff))]
    },
    { title: 'Reactivate User', emails: await mailer.reactivateUserT(mocks.email) },
    {
      title: 'Reset User Password',
      emails: await mailer.createForgotPasswordTokenT(mocks.email, 'TOKEN', mocks.id)
    },
    {
    ...
    },
    {
      ...
      emails: await mailer.updateDiscoveryDayToAttendeesT({
        vendor: mocks.publicVendor,
        rfi: mocks.rfi,
        attendees: [mocks.attendee(true), mocks.attendee(false)]
      })
    },
    {
  ...
  return () => {
    return (
      <html>
        ...
        >
          <a href="/" style={{ display: 'block', ...styles.classes.link, ...styles.utilities.mb[4] }}>
            Go back to the Procurement Concierge web app
          </a>
  ...
  return [
    {
      ...
      handler: {
        ...
        async respond(request): Promise<Response<HtmlResponseBody, unknown>> {
          ...
          if (!request.session || !permissions.isProgramStaff(request.session)) {
